function Frog(x, y) {
  ...
  this.move = function() {
    ...
    if(this.jumping()) {
      ...
      if(Math.sqrt(xDif * xDif + yDif * yDif) - player.size < 30 && !player.win) {
      ...
      if(this.timer > 120) {
        this.vx = Math.cos(this.d) * 2;
        this.vy = Math.sin(this.d) * 2  ;
        this.timer = 0;
  ...
  this.die = function() {
    ...
    if(this.opacity > .1) {
      ...
      let p = new Particle(this.x, this.y, (Math.random()-.5)*this.size * 2, (Math.random()-.5)*this.size * 2, 1, "hsla("+color+", 50%, 50%, "+this.opacity+")", .3, .98);
