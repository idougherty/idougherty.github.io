function Rabbit(x, y) {
  ...
  this.move = function() {
    ...
    if(this.jumping()) {
      ...
      if(this.timer > 40) {
        this.vx = Math.cos(this.d) * 3;
        this.vy = Math.sin(this.d) * 3;
        this.timer = 0;
  ...
  this.die = function() {
    ...
    if(this.opacity > .1) {
      ...
      let p = new Particle(this.x, this.y, (Math.random()-.5)*this.size * 2, (Math.random()-.5)*this.size * 2, 2, "hsla("+color+", 50%, 50%, "+this.opacity+")", .4, .98);
