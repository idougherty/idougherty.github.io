...
class PhysObject {
    ...
    constructor(pos, points, material = wood) {
        ...
        for(let i = 0; i < points.length; i++) {
            this.points[i] = Vec2D.rotate(Vec2D.ZERO, this.shape[i], this.angle);
...
class PhysEnv {
    ...
    detectCollisions(ctx) {
        ...
        for(let [s1, s2] of possibleCollisions) {
            if((simplex = this.GJK(s1, s2)) != false) {
                ...
                let contacts = this.findContacts(s1, s2, normal, ctx);
    ...
    resolveIntersections(s1, s2, normal, depth) {
        ...
        if(s1.mass == Infinity && s2.mass == Infinity) {
            ...
            s2.pos.x += normal.x * correction / 2;
            s2.pos.y += normal.y * correction / 2;
        } else if(s2.mass == Infinity) {
            s1.pos.x -= normal.x * correction / 2;
            s1.pos.y -= normal.y * correction / 2;
        } else {
    ...
    applyImpulses(s1, s2, normal, contact) {
        ...
        // const f1 = s1.vel.addRet(Vec2D.cross(s1.rotVel, r1));
        // const f2 = s2.vel.addRet(Vec2D.cross(s2.rotVel, r1));
        const vf = abVel;//Vec2D.dif(f1, f2);
    ...
    findContacts(s1, s2, normal, ctx) {
        ...
        // ctx.lineWidth = 6;
        // if(cp.length == 2)
        //     debugLine(cp[0], cp[1], ctx);
        // else if(cp.length == 1) {
        //     ctx.fillStyle = "red";
        //     ctx.fillRect(cp[0].x - 3, cp[0].y - 3, 6, 6);
        // }
        // ctx.lineWidth = 1;
    
        return cp;

        // const vec = Vec2D.dif(pRef, pInc);
        // const scalar = vec.dot(ref) / (ref.x * ref.x + ref.y * ref.y);
        // const proj = ref.mult(scalar);
        // let contact = new Vec2D(proj.x + pRef.x, proj.y + pRef.y);
