...
function Sqyrm(color, angle, dda) {
	...
	this.draw = function(x, y) {
		...
		for(var i = 0; i < sPoints.length-1; i++) {
			x1 = sPoints[i][0]*this.size;
			y1 = sPoints[i][1]*this.size;
			x2 = sPoints[i+1][0]*this.size;
			y2 = sPoints[i+1][1]*this.size;
			
			p1 = this.rotatePoint(x1, y1, x, y);
			p2 = this.rotatePoint(x2, y2, x, y);
...
function SqyrmRunner() {
	...
	this.update = function() {
		...
		for(var i = this.sqyrms.length-1; i >= 0; i--) {
			if(this.sqyrms[i].draw(canvas.width/2, canvas.height/2) == "kill me") {
